    ...
    const evolutionData = useMemo(() => {
        ...
        if (rangeStartDate && rangeEndDate) {
            ...
            while (temp <= rangeEndDate) {
                ...
                grouped[ds] = { date: ds, cv: 0, lp: 0, cp: 0, sharing: 0, holded: 0 };
                            ...
                            <AreaChart data={evolutionData}>
                                ...
                                <Tooltip
                                    ...
                                    formatter={(value: any, name: string | undefined) => {
                                        if (name && name.includes('HOLDED')) return [`${Number(value).toLocaleString('es-ES')}€`, name];
                                        return [value, name || ''];
                                    }}
                                />
                        ...
                        >
                            <div className="max-w-7xl mx-auto h-full flex flex-col">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {[
                                                ...
                                                <button
                                                    key={m.id}
                                                    onClick={() => {
                                                        if (m.id === 'docs') setShowInvoices(!showInvoices);
                                                        else setVisibleMetrics(prev => prev.includes(m.id) ? prev.filter(x => x !== m.id) : [...prev, m.id]);
                                                    }}
                                                    className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all ${m.id === 'docs' ? (showInvoices ? 'bg-indigo-600 text-white' : 'bg-white text-indigo-600 shadow-sm border border-indigo-100') : (visibleMetrics.includes(m.id) ? 'text-white' : 'bg-white text-gray-400 shadow-sm')}`}
                                                    style={{ backgroundColor: m.id !== 'docs' && visibleMetrics.includes(m.id) ? m.color : '' }}
                                                >
                                                    {m.label}
                                                </button>
                                            ))}
                                    ...
                                    {/* Export / Actions */}
                                    <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 md:col-span-1 flex flex-col justify-center">
                                        <p className="text-xs font-bold text-blue-900 mb-2">¿Necesitas una copia física?</p>
                                        <button className="bg-blue-600 text-white rounded-2xl px-6 py-3 text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all flex items-center justify-center space-x-2">
                                ...
                                <div className="flex-1 min-h-[500px] w-full bg-white rounded-[40px] border border-gray-100 shadow-2xl p-8 relative overflow-hidden group">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={evolutionData}>
                                            <defs>
                                                <linearGradient id="colorLP_exp" x1="0" y1="0" x2="0" y2="1">
                                                    <stop offset="5%" stopColor={COLORS.lp} stopOpacity={0.6} />
                                                    <stop offset="95%" stopColor={COLORS.lp} stopOpacity={0} />
                                                </linearGradient>
                                                <linearGradient id="colorCP_exp" x1="0" y1="0" x2="0" y2="1">
                                                    <stop offset="5%" stopColor={COLORS.cp} stopOpacity={0.6} />
                                                    <stop offset="95%" stopColor={COLORS.cp} stopOpacity={0} />
                                                </linearGradient>
                                                <linearGradient id="colorCV_exp" x1="0" y1="0" x2="0" y2="1">
                                                    <stop offset="5%" stopColor={COLORS.cv} stopOpacity={0.6} />
                                                    <stop offset="95%" stopColor={COLORS.cv} stopOpacity={0} />
                                                </linearGradient>
                                            </defs>
                                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                                            <XAxis
                                                ...
                                                axisLine={false}
                                                tickLine={false}
                                                tick={{ fontSize: 12, fill: '#0F1D42', fontWeight: 800 }}
                                            />
                                            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#0F1D42', fontWeight: 800 }} />
                                            <Tooltip
                                                contentStyle={{ borderRadius: '30px', border: 'none', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', padding: '25px', backdropFilter: 'blur(10px)', backgroundColor: 'rgba(255,255,255,0.9)' }}
                                                itemStyle={{ fontSize: '14px', fontWeight: '900', textTransform: 'uppercase' }}
                                                cursor={{ stroke: '#3B82F6', strokeWidth: 2, strokeDasharray: '10 10' }}
                                            />
                                            <Area
                                                type="monotone"
                                                dataKey="lp"
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 text-gray-400 text-[10px] uppercase font-black tracking-widest">
                                    ...
                                    <th className="p-6 text-center border-b border-gray-100 bg-emerald-50/20 text-emerald-700">Ganancia (Propia)</th>
