                                    ...
                                    <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 md:col-span-1">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {[
                                                ...
                                                <button
                                                    key={m.id}
                                                    onClick={() => setVisibleMetrics(prev => prev.includes(m.id) ? prev.filter(x => x !== m.id) : [...prev, m.id])}
                                                    className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all ${visibleMetrics.includes(m.id) ? 'text-white' : 'bg-white text-gray-400 shadow-sm'}`}
                                                    style={{ backgroundColor: visibleMetrics.includes(m.id) ? m.color : '' }}
                                                >
                                                    {m.label}
                                                </button>
                                            ))}
